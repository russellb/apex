...
.PHONY: srpm
srpm: dist/rpm ## Build a source RPM
	go mod vendor
	...
	mock --buildsrpm -D "_commit ${APEX_RELEASE}" --resultdir=$(CURDIR)/dist/rpm/mock --spec contrib/rpm/apex.spec --sources $(CURDIR)/dist/rpm/
	rm -f dist/rpm/apex-${APEX_RELEASE}.tar.gz

.PHONY: rpm
rpm: srpm ## Build an RPM
	mock --rebuild --without check --resultdir=$(CURDIR)/dist/rpm/mock $(wildcard dist/rpm/mock/apex-*$(shell date +%Y%m%d)*$(APEX_RELEASE)*.src.rpm)
