...
func proxyTypeStr(ruleType int) (string, error) {
	switch ruleType {
...
func proxyProtocol(protocol string) (string, error) {
...
func (ax *Nexodus) UserspaceProxyAdd(ctx context.Context, wg *sync.WaitGroup, proxyRule string, ruleType int) error {
...
func (proxy *UsProxy) run(ctx context.Context, proxyWg *sync.WaitGroup) error {
	...
	if proxy.ruleType == ProxyTypeEgress {
		l, err = net.Listen(proxy.protocol, fmt.Sprintf(":%d", proxy.listenPort))
	} else {
...
func (proxy *UsProxy) handleConnection(ctx context.Context, proxyWg *sync.WaitGroup, inConn net.Conn) error {
	...
	var err error
	if proxy.ruleType == ProxyTypeEgress {
		outConn, err = proxy.userspaceNet.DialContext(ctx, proxy.protocol, proxyDest)
	} else {
		outConn, err = net.Dial(proxy.protocol, proxyDest)
	}
	if err != nil {
		return err
	}
